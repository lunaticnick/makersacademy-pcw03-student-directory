...
def print (students)
  ...
  if selection == "y"
      students.each_with_index do |student, index|
        puts "#{index + 1}. #{student[:name]}".ljust($page_width/ 3) + " || #{student[:cohort]} cohort".ljust($page_width/3) + " || Birthday: #{student[:birthday]}".ljust($page_width/3) + " || Nationality: #{student[:nationality]}".ljust($page_width/3)
